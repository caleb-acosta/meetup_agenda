defmodule MeetupAgendaWeb.EventLive.Index do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:events, list_events(socket.assigns.filter_month.year, socket.assigns.filter_month.month))
  end
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    ...
    {:noreply, assign(socket, events: list_events(socket.assigns.filter_month.year, socket.assigns.filter_month.month))}
  end
  ...
  def handle_event("prev_month", _, socket) do
    filter_month = cond do
      socket.assigns.filter_month.month == 1 -> %{month: 12, year: socket.assigns.filter_month.year - 1}
      true -> Map.update!(socket.assigns.filter_month, :month, &(&1 - 1))
    end
    {:noreply, socket
   ...
   def handle_event("next_month", _, socket) do
    filter_month = cond do
      socket.assigns.filter_month.month == 12 -> %{month: 1, year: socket.assigns.filter_month.year + 1}
      true -> Map.update!(socket.assigns.filter_month, :month, &(&1 + 1))
    end
    {:noreply, socket
      |> assign(filter_month: filter_month)
